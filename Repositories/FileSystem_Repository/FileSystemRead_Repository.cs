...
public class FileSystemRead_Repository : IFileSystemReadService
{
      ...
      CancellationToken ct = default)
  {
    try
    {
      ...
      if (Directory.Exists(customerPath))
      {
        ...
        foreach (var dirPath in subDirectories)
        {
          ...
          var filesInFolder = space.FileResources
    ...
    {
      _logger.LogError(ex, "Error al obtener estructura de carpetas para cliente {CustomerId}", customerId);
      return new ApiResponse<CustomerFolderStructureDTO>(
          false,
      ...
      CancellationToken ct = default)
  {
    ...
    {
      _logger.LogError(ex, "Error al obtener contenido de carpeta {FolderName} para cliente {CustomerId}", folderName, customerId);
      return new ApiResponse<FolderContentDTO>(
          false,
