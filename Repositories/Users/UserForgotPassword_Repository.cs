...
public class UserForgotPassword_Repository : IUserForgotPassword
{
    ...
    public UserForgotPassword_Repository(
        UserPassword_Repository repo,
        IEmailService mail,
        ITokenService token
    )
    {
    ...
    )
    {
        ...
        if (user is null)
            return new(false, "Invalid or expired token");
        ...
        await _mail.SendPasswordChangedAsync(
            user.Email,
            $"Your password was changed from {ipAddress} at {DateTime.UtcNow:g}"
        );
    ...
    public async Task<ApiResponse<string>> SendOtpAsync(string email, string token)
    {
        ...
        await _mail.SendOtpAsync(user.Email, $"Your OTP code is: {user.Otp}");
