...
public class PermissionsRead_Repository : IReadCommandPermissions
{
    ...
    public PermissionsRead_Repository(
        ApplicationDbContext context,
        ILogger<PermissionsRead_Repository> log,
        IMapper mapper
    )
    {
        _context = context;
    ...
    public async Task<ApiResponse<List<PermissionsDTO>>> GetAll()
    {
        try
        {
            ...
            if (listPermissions.Count > 0)
            {
                ...
                return new ApiResponse<List<PermissionsDTO>>(
                    true,
                    $"Permissions Founded {listPermissions.Count}",
                    found
                );
            }
            else
    ...
    public async Task<ApiResponse<PermissionsDTO>> GetById(Guid PermissionId)
    {
        try
        {
            var permission = await _context.Permissions.FirstOrDefaultAsync(r =>
                r.Id == PermissionId
            );
        ...
        catch (Exception ex)
        {
            ...
            return new ApiResponse<PermissionsDTO>(
                false,
                "An error ocurred while retrieving the permission",
                null
            );
        }
    }
