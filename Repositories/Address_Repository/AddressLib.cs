...
public class AddressLib : IAddress
{
    ...
    public async Task<ApiResponse<bool>> AddNew(AddressDTOS addressDTO, CancellationToken cancellationToken = default)
    {
        try
        {
            ...
            if (string.IsNullOrEmpty(addressDTO.City) || string.IsNullOrEmpty(addressDTO.Street) || string.IsNullOrEmpty(addressDTO.Line) || string.IsNullOrEmpty(addressDTO.ZipCode))
            {
            ...
            var userExists = await _context.User.AnyAsync(u => u.Id == addressDTO.UserId, cancellationToken);
            if (!userExists)
            {
            ...
            var countryExists = await _context.Country.AnyAsync(c => c.Id == addressDTO.CountryId, cancellationToken);
            if (!countryExists)
            {
            ...
            var stateExists = await _context.State.AnyAsync(s => s.Id == addressDTO.StateId, cancellationToken);
            if (!stateExists)
            {
            ...
            var addressExists = await _context.Address.AnyAsync(a => a.UserId == addressDTO.UserId, cancellationToken);
            if (addressExists)
            {
                _log.LogError("Address already exists for user with ID {UserId}.", addressDTO.UserId);
            ...
            if (result)
            {
                _log.LogInformation("Address added successfully for user with ID {UserId}.", addressDTO.UserId);
                return new ApiResponse<bool>(true, "Address added successfully.", true);
            }
            else
            {
                _log.LogError("Failed to add address for user with ID {UserId}.", addressDTO.UserId);
    ...
    public async Task<ApiResponse<bool>> Delete(int id, CancellationToken cancellationToken = default)
    {
        ...
        catch (Exception ex)
        {
            ...
            // Log the exception or handle it as needed
            _log.LogError("An error occurred while deleting address with ID {Id}: {ex.Message}", id, ex);
            return new ApiResponse<bool>(false, ex.Message, false);
        }
    }

    public async Task<ApiResponse<bool>> Exists(UserDTO user, CancellationToken cancellationToken = default)
    {
        try
        {
            var exists = await _context.Address.AnyAsync(x => x.UserId == user.Id, cancellationToken);
            if (exists == true)
            {
        ...
        {
            // Log the exception or handle it as needed
            _log.LogError("An error occurred while checking if the address exists for user {UserId}", ex);
            return new ApiResponse<bool>(false, ex.Message, false);
        }
    }

    public async Task<ApiResponse<List<AddressDTObyUser>>> GetAll(CancellationToken cancellationToken = default)
    {
        try
        {
            List<AddressDTObyUser> query = await (from a in _context.Address
            ...
            return new ApiResponse<List<AddressDTObyUser>>(true, "Addresses retrieved successfully.", query);
    ...
    public async Task<ApiResponse<AddressDTOS>> GetById(int id, CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            return new ApiResponse<AddressDTOS>(true, "Address retrieved successfully.", addressDTO);
        }
        catch (Exception ex)
        {
            // Log the exception or handle it as needed
            _log.LogError("An error occurred while retrieving address with ID {Id}: {ex.Message}", id, ex);
    ...
    public Task<ApiResponse<bool>> Update(AddressDTOS addressDTO, CancellationToken cancellationToken = default)
    {
