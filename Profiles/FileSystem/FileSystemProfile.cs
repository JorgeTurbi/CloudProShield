...
public class FileSystemProfile : Profile
{
    public FileSystemProfile()
    {
        CreateMap<FileResource, FileItemDTO>()
            .ForMember(
                dest => dest.Category,
                opt => opt.MapFrom(src => GetCategoryFromPath(src.RelativePath))
            )
        ...
        CreateMap<SpaceCloud, SpaceCloudDTO>().ReverseMap();
        CreateMap<FileResourceCloud, FileResourceCloudDTO>().ReverseMap();
    }
